...
function handleTemplateImageUpload(req, res, next) {
  uploadTemplateImage.single("image")(req, res, (err) => {
    if (err) {
...
function deleteTemplateImage(imageUrlOrFilename) {
  ...
  try {
    ...
    console.error("Failed to delete template image:", err.message);
  }
}
